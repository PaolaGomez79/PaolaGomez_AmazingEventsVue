  ...
  created() {
    fetch("https://mindhub-xj03.onrender.com/api/amazing")
      ...
      .then((data) => {
        ...
        this.upComingEvents = this.dataEvents.filter(event => new Date(event.date) > new Date(this.datos.currentDate));
        this.checkBoxes = [...new Set(this.upComingEvents.map(event => event.category))];
        this.temaOscuro();
      })
  ...
  computed: {
    filtrarPorCheckBox: function filtroCheckBox() {
      let eventChecked = this.checked.length == 0
          ? this.upComingEvents
          : this.upComingEvents.filter(event => this.checked.includes(event.category));
