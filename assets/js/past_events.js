  ...
  created() {
    fetch("https://mindhub-xj03.onrender.com/api/amazing")
      ...
      .then((data) => {
        ...
        this.pastEvents = this.dataEvents.filter(event => new Date(event.date) < new Date(this.datos.currentDate));
        this.checkBoxes = [...new Set(this.pastEvents.map(event => event.category))];
        this.temaOscuro();
      })
  ...
  computed: {
    filtrarPorCheckBox: function filtroCheckBox() {
      let eventChecked = this.checked.length == 0
          ? this.pastEvents
          : this.pastEvents.filter(event => this.checked.includes(event.category));
...
.then(data => {
  ...
  const card_pas = document.getElementById('cardsPastEvents');
  showCard(past_events, card_pas);
  temaOscuro();

  const contenedorCheck = document.getElementById('container-form-check');
  recorrerCardsYRenderizarNombreCategoria(past_events, contenedorCheck);  
  ...
  checkboxes.forEach(checkbox => {
      checkbox.addEventListener('change', () => {
        ...
        filterCruzade(past_events, inputsChequeados, stringSearch,card_pas);
        temaOscuro()
      })
  });
...
const recorrerCardsYRenderizarNombreCategoria = (array, contenedor) => {
  ...
  console.log(listaCategoriasUnicas);
  let fragment = document.createDocumentFragment();
...
function filterArray(arrayString, listEvents) {
    ...
    return listEvents.filter(elemento => arrayString.includes(elemento.category.replace(" ", "_")))
};


function filterString(string, listEvents) {
    if (string == "") return listEvents
    return listEvents.filter(elemento => elemento.name.toLowerCase().includes(string.toLowerCase().trim()))
};
